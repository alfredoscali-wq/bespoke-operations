...
test("14-16: list, detail y summary usan la misma semántica de display", () => {
  ...
  const tally = read("lib/network/monitoring/summary-tally.ts")
  ...
  assert.match(tally, /displayMonitoringStatus\(/)
  assert.match(tally, /displayed === "online"/)
  assert.match(tally, /displayed === "offline"/)
  assert.doesNotMatch(tally, /if \(row\.status === "online"\)/)
  assert.doesNotMatch(tally, /if \(row\.status === "offline"\)/)
