...
for (const status of BLOCKING_STATUSES) {
  test(`finalizar obra bloqueada con OT ${status}`, () => {
    ...
    if (!result.ok) {
      assert.match(result.message, /órdenes de trabajo abiertas/i)
    }
  })
}
...
test("finalizar obra: estado inválido de project rechazado", () => {
  const result = validateFinalizeProject({
...
test("mensaje de bloqueo incluye cantidad de OT pendientes", () => {
  ...
  assert.match(message, /órdenes de trabajo abiertas/i)
})

test("historial de finalización describe transición", () => {
  assert.match(
...
test("parseFinalizeProjectRpcResult acepta respuesta válida", () => {
...
test("parseFinalizeProjectRpcResult rechaza respuesta inválida", () => {
...
test("migración finalize_project_operational: FOR UPDATE y multi-tenant", () => {
  ...
  assert.match(sql, /órdenes de trabajo abiertas/)
  assert.doesNotMatch(sql, /UPDATE public\.tasks/)
  assert.doesNotMatch(sql, /GRANT EXECUTE[\s\S]*TO authenticated/)
  assert.match(sql, /GRANT EXECUTE[\s\S]*TO service_role/)
})

test("migración: atomicidad — solo actualiza project si no hay OT abiertas", () => {
  const sql = readFileSync(MIGRATION_PATH, "utf8")
...
test("API finalize: sesión writable + módulo projects + prefetch tenant", () => {
...
test("reapertura closed → active sin regresión", () => {
...
test("acción Ver disponible para todas las OT de obra", () => {
  for (const status of [
    ...BLOCKING_STATUSES,
...
test("acción Editar programada disponible", () => {
  const actions = resolveProjectTaskRowActions(
...
test("acción Editar asignada disponible", () => {
  const actions = resolveProjectTaskRowActions(
...
test("acción Editar en-curso no disponible", () => {
  const actions = resolveProjectTaskRowActions(
    makeObraTask({ status: "en-curso" })
  )
...
test("acción Eliminar programada disponible", () => {
  const actions = resolveProjectTaskRowActions(
    makeObraTask({ status: "programada" })
  )
...
test("acción Eliminar asignada no disponible", () => {
  const actions = resolveProjectTaskRowActions(
    makeObraTask({ status: "asignada" })
  )
...
test("acción Eliminar estados posteriores no disponible", () => {
  for (const status of [
    "vencida",
    "en-curso",
    "incidencia",
...
test("en-aprobacion también muestra Revisar cierre", () => {
  const actions = resolveProjectTaskRowActions(
...
test("aprobar cierre: pendiente-cierre → finalizada", () => {
...
test("rechazar cierre: pendiente-cierre → en-curso", () => {
  const transition = getTransitionForAction("reject")
  assert.ok(transition.from.includes("pendiente-cierre"))
  assert.equal(transition.to, "en-curso")
})

test("OT de obra no aplica side-effects de Planificación", () => {
  assert.equal(
...
test("OT normales sin project_id mantienen reglas de edición admin", () => {
  assert.equal(
...
test("sheet de revisión reutiliza PlanningPendingClosureDetailPanel", () => {
