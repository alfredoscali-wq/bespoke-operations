import {
  getMobileAgendaToday,
  validateMobileAgendaTodayQuery,
} from "@/lib/mobile/v1/agenda/agenda-service"
...
export async function GET(request: Request) {
  return handleProtectedMobileRoute(request, async (context) => {
    ...
    const query = validateMobileAgendaTodayQuery(url.searchParams.get("deviceId"))
    const result = await getMobileAgendaToday(context.auth, query.deviceId)
