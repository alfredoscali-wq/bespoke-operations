import { finishMobileShift } from "@/lib/mobile/v1/shifts/shift-service"
import { validateMobileShiftFinishRequest } from "@/lib/mobile/v1/shifts/validate-shift-request"
...
export async function POST(request: Request) {
  return handleProtectedMobileRoute(request, async (context) => {
    ...
    const finishRequest = validateMobileShiftFinishRequest(body)
    const result = await finishMobileShift(context.auth, finishRequest)
