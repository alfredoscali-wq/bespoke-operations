import {
  startMobileShift,
} from "@/lib/mobile/v1/shifts/shift-service"
import { validateMobileShiftStartRequest } from "@/lib/mobile/v1/shifts/validate-shift-request"
...
export async function POST(request: Request) {
  return handleProtectedMobileRoute(request, async (context) => {
    ...
    const startRequest = validateMobileShiftStartRequest(body)
    const result = await startMobileShift(context.auth, startRequest)
