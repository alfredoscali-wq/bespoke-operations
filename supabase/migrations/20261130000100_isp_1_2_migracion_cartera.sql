-- ISP 1.2 — Initial portfolio migration (staging → validate → transactional import).
-- Additive only. Does not rebuild portfolio from historical work orders.
-- Does not alter Tesorería, Atención al Cliente or Archivo OT.
...
COMMENT ON COLUMN public.isp_service_catalog.external_code IS
  'ISP-provided catalog identifier used during portfolio migration.';
COMMENT ON COLUMN public.isp_services.external_code IS
  'ISP-provided contracted-service identifier used during portfolio migration.';
COMMENT ON COLUMN public.isp_connections.external_code IS
  'ISP-provided connection identifier used during portfolio migration.';

CREATE UNIQUE INDEX IF NOT EXISTS isp_service_catalog_company_external_code_idx
  ON public.isp_service_catalog (company_id, lower(external_code))
  WHERE deleted_at IS NULL AND external_code IS NOT NULL AND btrim(external_code) <> '';

CREATE UNIQUE INDEX IF NOT EXISTS isp_services_company_external_code_idx
  ON public.isp_services (company_id, lower(external_code))
  WHERE deleted_at IS NULL AND external_code IS NOT NULL AND btrim(external_code) <> '';

CREATE UNIQUE INDEX IF NOT EXISTS isp_connections_company_external_code_idx
  ON public.isp_connections (company_id, lower(external_code))
  WHERE deleted_at IS NULL AND external_code IS NOT NULL AND btrim(external_code) <> '';

CREATE TABLE IF NOT EXISTS public.isp_company_settings (
  company_id uuid PRIMARY KEY REFERENCES public.companies (id),
  onboarding_cutoff_at timestamptz,
  updated_at timestamptz NOT NULL DEFAULT now(),
  updated_by uuid
);

COMMENT ON TABLE public.isp_company_settings IS
  'Persistent ISP operation settings. onboarding_cutoff_at is when Bespoke starts managing new ISP altas.';
COMMENT ON COLUMN public.isp_company_settings.onboarding_cutoff_at IS
  'Work orders before this timestamp remain operational history and must not generate ISP altas.';

CREATE TABLE IF NOT EXISTS public.isp_connection_equipment (
...
CREATE INDEX IF NOT EXISTS isp_connection_equipment_company_idx
  ON public.isp_connection_equipment (company_id)
  WHERE deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS isp_connection_equipment_company_external_code_idx
  ON public.isp_connection_equipment (company_id, lower(external_code))
  WHERE deleted_at IS NULL AND external_code IS NOT NULL AND btrim(external_code) <> '';

CREATE TABLE IF NOT EXISTS public.isp_migration_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies (id),
  ...
  imported_customers_count integer NOT NULL DEFAULT 0,
  imported_catalog_count integer NOT NULL DEFAULT 0,
  imported_services_count integer NOT NULL DEFAULT 0,
  imported_connections_count integer NOT NULL DEFAULT 0,
  imported_equipment_count integer NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS isp_migration_runs_company_idx
  ON public.isp_migration_runs (company_id, started_at DESC);

CREATE INDEX IF NOT EXISTS isp_migration_runs_company_hash_idx
  ON public.isp_migration_runs (company_id, file_sha256)
  WHERE file_sha256 IS NOT NULL;

CREATE TABLE IF NOT EXISTS public.isp_migration_staging_rows (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  run_id uuid NOT NULL REFERENCES public.isp_migration_runs (id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies (id),
  sheet text NOT NULL,
...
CREATE INDEX IF NOT EXISTS isp_migration_staging_run_sheet_idx
  ON public.isp_migration_staging_rows (run_id, sheet, row_number);

COMMENT ON TABLE public.isp_migration_runs IS
  'Audit trail of ISP portfolio migration runs. Preview/validate never writes final ISP tables.';
COMMENT ON TABLE public.isp_migration_staging_rows IS
  'Validated spreadsheet rows. PPPoE passwords in payload must not be returned to clients or written to logs.';

ALTER TABLE public.isp_company_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.isp_connection_equipment ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.isp_migration_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.isp_migration_staging_rows ENABLE ROW LEVEL SECURITY;

CREATE POLICY isp_company_settings_select_policy
  ...
  USING (
    ...
    AND (
      ...
      OR public.auth_user_has_allowed_module('maintenance')
    )
  );

CREATE POLICY isp_company_settings_upsert_policy
  ON public.isp_company_settings
  FOR ALL
  USING (
    company_id = public.auth_user_company_id()
...
CREATE POLICY isp_connection_equipment_select_policy
  ON public.isp_connection_equipment
  FOR SELECT
  USING (
    company_id = public.auth_user_company_id()
...
CREATE POLICY isp_connection_equipment_write_policy
  ON public.isp_connection_equipment
  FOR ALL
  USING (
    company_id = public.auth_user_company_id()
...
CREATE POLICY isp_migration_runs_select_policy
...
CREATE POLICY isp_migration_runs_write_policy
  ON public.isp_migration_runs
  FOR ALL
  USING (
    company_id = public.auth_user_company_id()
...
CREATE POLICY isp_migration_staging_select_policy
...
CREATE POLICY isp_migration_staging_write_policy
  ON public.isp_migration_staging_rows
  FOR ALL
  USING (
    company_id = public.auth_user_company_id()
...
GRANT SELECT, INSERT, UPDATE ON public.isp_company_settings TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.isp_connection_equipment TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.isp_migration_runs TO authenticated;
