-- Hotfix: create_task_with_execution_order extracted scheduled_time as text
-- (jsonb ->>) and Postgres rejected INSERT into TIME WITHOUT TIME ZONE.
-- Cast the normalized value. Column type, due_date, start_date, execution_order
-- and OT 1.1–1.5.1 behaviour are unchanged. Empty/null stay NULL.
...
COMMENT ON FUNCTION public.create_task_with_execution_order(jsonb) IS
  'Creates a task and assigns execution_order atomically for company+crew+due_date. Client execution_order is ignored. Unique index tasks_execution_order_crew_date_unique remains the last barrier. scheduled_time is cast from jsonb text to TIME.';
