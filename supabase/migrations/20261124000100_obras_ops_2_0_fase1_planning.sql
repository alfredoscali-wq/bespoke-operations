...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    IF NEW.project_id IS NOT NULL THEN
      ...
      -- OPS 2.0: obra activa → entra a Planificación como programada.
      IF v_project_status = 'active'::public.project_status THEN
        NEW.status := 'programada'::public.task_status;
        RETURN NEW;
      END IF;

      -- Obra no iniciada → siempre borrador (fuera de Planificación / Mobile).
...
BEGIN
  ...
  WHERE t.project_id = p_project_id
    AND t.company_id = p_company_id
    AND t.deleted_at IS NULL
    AND t.status = 'borrador'::public.task_status
  ...
  IF v_dispatched_count = 1 THEN
    v_history_description :=
      'Estado actualizado de Planificada a Activa. Despacho operativo: 1 tarea pasó a Programada e ingresó a Planificación.';
  ELSIF v_dispatched_count > 1 THEN
    v_history_description := format(
      'Estado actualizado de Planificada a Activa. Despacho operativo: %s tareas pasaron a Programada e ingresaron a Planificación.',
      v_dispatched_count
    );
  ELSE
    v_history_description :=
      'Estado actualizado de Planificada a Activa. No había tareas en borrador pendientes de ingreso a Planificación.';
  END IF;
...
COMMENT ON FUNCTION public.start_project_operational_dispatch(uuid, uuid, text) IS
  'OBRAS OPS 2.0: start planned project and promote borrador tasks to programada (Planificación). service_role only.';
...
UPDATE public.tasks t
SET
  status = 'borrador'::public.task_status,
  updated_at = now()
FROM public.projects p
WHERE t.project_id = p.id
  ...
  AND p.deleted_at IS NULL
  AND t.deleted_at IS NULL
  AND p.status = 'planned'::public.project_status
