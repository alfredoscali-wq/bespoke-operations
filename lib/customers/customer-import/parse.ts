...
} from "@/lib/customers/customer-import/columns"
import {
  cellToString,
  resolveImportStatus,
...
const DATA_SHEET_NAMES = ["clientes", "customers"]

function isRowEmpty(data: CustomerImportRowData): boolean {
...
function applyFieldValue(
  ...
  switch (field) {
    case "technology":
      ...
      break
    case "status":
      data.status = resolveImportStatus(rawValue)
      break
    default:
...
function rowArrayToData(
  cells: unknown[],
  columnMap: Partial<Record<number, CustomerImportField>>
...
export async function parseCustomerImportFile(
  file: File
): Promise<{ rows: { rowNumber: number; data: CustomerImportRowData }[]; fileName: string }> {
  ...
  const rows: { rowNumber: number; data: CustomerImportRowData }[] = []
