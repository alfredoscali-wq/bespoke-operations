...
import { resolveConsultationInboxHeading } from "@/lib/customer-atenciones/consultation-inbox-heading"
...
export function ConsultationInboxSection({
  ...
  const heading = resolveConsultationInboxHeading(query)
      ...
      <CardHeader className="space-y-3 pb-2 pt-4">
        <div className="flex flex-col gap-0.5">
          <CardTitle className="text-[15px] font-semibold tracking-tight text-foreground sm:text-base">
            {isSharedInboxLoading ? "Bandeja de Consultas" : heading.title}
          </CardTitle>
          <p className="text-xs text-muted-foreground">
            {isSharedInboxLoading ? "Cargando…" : heading.subtitle}
          </p>
        </div>
